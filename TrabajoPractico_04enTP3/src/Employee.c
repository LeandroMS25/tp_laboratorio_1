...
void funcionImprimir(void* pElement)
{
	...
	Employee* auxEmp = (Employee*)pElement;
	employee_allGets(auxEmp, &bufferId, bufferNombre, &bufferHorasTrabajadas, &bufferSueldo);
	printf("Id: %d | Nombre: %s | Horas: %d | Sueldo: %d\n",bufferId,bufferNombre,bufferHorasTrabajadas,bufferSueldo);
}
int funcionFiltrar(void* thisA)
{
	int retorno = -1;
	Employee* auxEmp = thisA;
	char bufferNombre[LEN_NAME];

	if(thisA != NULL && employee_getNombre(auxEmp, bufferNombre) == 0 && bufferNombre[0] != 'A')
	{
		retorno = 1;
	}
	return retorno;
